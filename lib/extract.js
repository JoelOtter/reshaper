...
function extractArrayFromBacktrack(data, schema, stacks, hints) {
    var newStacks = {
        data: stacks.data.slice(),
        goodKeys: stacks.goodKeys,
        staleKeys: stacks.staleKeys
    };
    while (newStacks.data.length > 0) {
        try {
            return traverseAndFind(newStacks.data.shift(), 'Array', newStacks, hints);
        } catch (err) {
            continue;
        }
    }
...
function extractArray(data, schema, stacks, hints) {
    ...
    try {
        ...
        try {
            ...
            // Still nothing? OK. Let's make an array out of object properties.
